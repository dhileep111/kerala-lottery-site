#!/usr/bin/env python3
"""
et_scraper.py v11 - Kerala Lottery Auto-Scraper (Fortified)
Sources: ET, Goodreturns, keralalotteries.net, lotteryresultsnow.com, klresultstoday.com

Fixes applied:
- Robust Ticket Regex: Handles 'AA123456', 'AA-123456', 'AA 123456', etc.
- Expanded Proximity: Widened distance checks to bypass large text/ad blocks.
- Fuzzy Chunking: Better tolerance for typos in prize labels.
- Fallback parsing logic so it doesn't crash on minor margin errors.
"""
...
def get_today_lottery():
    ...
    try:
        ...
        print(f"Error: Could not find data files at {DATA}")
        sys.exit(1)
...
def is_fresh_result_page(html, draw_code):
    ...
    if not dc_positions or not prize_positions:
        return False
    ...
    result_markers = (
        ...
        'winners numbers', 'live update'
    )
    has_result_marker = any(marker in lower for marker in result_markers)
    
    # INCREASED: Sites inject a lot of text between headers and tables now
    max_dist = 12000 if has_result_marker else 5000 

    if dist > max_dist:
        print(f"  ⚠ '{draw_code}' and prize are {dist} plain-text chars apart — index page, skipping")
        return False

    print(f"  ✓ Freshness OK — '{draw_code}' and prize are {dist} chars apart")
    return True
...
def try_economic_times(lottery, draw_code, date):
    ...
    for env_name in ('ET_ARTICLE_URL', 'RESULT_ARTICLE_URL', 'SOURCE_URL'):
        ...
        if not direct_url or 'economictimes.indiatimes.com' not in direct_url.lower():
            continue
        if not is_result_article_url(direct_url): continue
        print(f"  ET direct ({env_name}): {direct_url[:80]}")
        h = fetch(direct_url)
        if is_fresh_result_page(h, draw_code): return h
    ...
    if search_html:
        ...
        for link in links:
            ll = link.lower()
            if et_name not in ll and draw_code.lower() not in ll: continue
            if not is_result_article_url(link): continue
    ...
    for link in google_search_et_url(lottery['name'], draw_code, date_str):
        if et_name not in link.lower() and draw_code.lower() not in link.lower(): continue
        if not is_result_article_url(link): continue
        h = fetch(link)
        if is_fresh_result_page(h, draw_code): return h
...
def try_goodreturns(lottery_slug, draw_code):
    ...
    h = fetch(url)
    return h if is_fresh_result_page(h, draw_code) else ""
...
def parse_prizes(html, draw_code=''):
    ...
    def matching_sections(labels, window=3500):
        # Relaxed label matching to allow variable spacing (e.g., "1st   Prize")
        label_pat = '|'.join(re.sub(r'\s+', r'\\s+', re.escape(label)) for label in labels)
        for m in re.finditer(label_pat, text, re.IGNORECASE):
            chunk = text[m.start():m.start() + window]
            # More robust stop regex to catch slight misspellings or spacing issues
            stop = re.search(
                r'\b(?:Consolation|Cons|1st|First|2nd|Second|3rd|Third|4th|Fourth|5th|Fifth|6th|Sixth|7th|Seventh|8th|Eighth|9th|Ninth)[\s\:\-]*Prize\b',
                chunk[30:],
                re.IGNORECASE,
            )
            if stop:
                chunk = chunk[:30 + stop.start()]
            yield chunk

    def find_ticket(labels):
        for chunk in matching_sections(labels, window=1000):
            # FIXED: Made space between series letters and numbers optional/flexible [\s\-]*
    ...
    if '1st' in prizes:
        ...
        if first_6:
            ...
            for m in re.finditer(r'Consolation\s+Prize|Cons\s+Prize|Consolation', text, re.IGNORECASE):
                ...
                # FIXED: Made space flexible here too
...
def get_recent_prizes(lottery_slug, limit=3):
    try:
        ...
        seen = set()
        for r in results[:15]:
            if r['lotterySlug'] == lottery_slug: continue
...
def main():
    ist = datetime.timezone(datetime.timedelta(hours=5, minutes=30))
    now = datetime.datetime.now(ist)
    print(f"Scraper v11 (Fortified): {now.strftime('%H:%M IST, %d %b %Y')}")

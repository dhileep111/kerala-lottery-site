...
import type { Result, Prize } from '../types';

// ── Kerala ticket format helpers ──────────────────────────
// Full ticket: "RR 281074" — series (2 letters) + space + 6-digit number
// Partial (4-digit): "1234" — matches last 4 digits of any series
...
interface MatchResult {
  ...
  amount:    string;
  number:    string;
  isExact:   boolean;  // full ticket match
  isPartial: boolean;  // last-4 match
  drawCode:  string;
  drawDate:  string;
  lottery:   string;
  slug:      string;
...
function searchAllResults(raw: string): MatchResult[] {
  ...
  for (const result of results) {
    ...
    for (const prize of result.prizes) {
      for (const num of prize.numbers) {
        ...
        // Case 1: exact full ticket match e.g. "RR 281074"
        if (fullMatch) {
          const [, series, digits] = fullMatch;
          const [tSeries, tDigits] = ticketNorm.split(' ');
          if (tSeries === series.toUpperCase() && tDigits === digits) {
            found.push({
              tier: prize.tier, amount: prize.amount,
              number: ticketNorm, isExact: true, isPartial: false,
              drawCode: result.drawCode, drawDate: result.displayDate,
              lottery: lotteryName, slug: result.lotterySlug,
            });
          }
          continue;
        }

        // Case 2: 4-digit partial match (last 4 of 6-digit number)
        if (is4digit) {
          const digits = ticketNorm.replace(/[A-Z\s]/g, '');
          if (digits.endsWith(query) || digits.slice(-4) === query) {
            found.push({
              tier: prize.tier, amount: prize.amount,
              number: ticketNorm, isExact: false, isPartial: true,
              drawCode: result.drawCode, drawDate: result.displayDate,
              lottery: lotteryName, slug: result.lotterySlug,
            });
          }
        ...
        // Case 3: 6-digit partial (no series) — match the number part
        if (is6digit) {
          const digits = ticketNorm.replace(/[A-Z\s]/g, '');
          if (digits === query) {
            found.push({
        ...
        // Case 4: generic substring (series partial etc.)
  ...
  // Deduplicate & sort: exact first, then by draw date desc
    ...
    <div style={{
      ...
      {is1st && (
        <div style={{
          ...
          borderRadius: '0 0 10px 10px', letterSpacing: '0.05em',
        }}>JACKPOT</div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 22, marginBottom: 4 }}>{style.icon}
            <span style={{ fontSize: 17, fontWeight: 700, color: style.textColor, marginLeft: 8 }}>
              {match.tier}
            </span>
          </div>
          <div style={{ fontFamily: 'monospace', fontSize: 20, fontWeight: 800, color: style.textColor, letterSpacing: '0.08em' }}>
            {match.number}
          </div>
          <div style={{ fontSize: 13, color: '#6b7280', marginTop: 6 }}>
            {match.lottery} • {match.drawCode} • {match.drawDate}
          </div>
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 10 }}>
        📋 How to enter your ticket number
      </div>
...
function TamilGuide() {
  return (
    <div style={{
      ...
      fontSize: 14, lineHeight: 1.9, color: '#880e4f',
    }}>
...
export default function CheckTicketPage() {
  ...
  const handleClear = () => {
    setInput(''); setQuery(''); setChecked(false);
  };
    ...
    <main className="page">
      <div className="container" style={{ maxWidth: 720 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            ...
            <button
              ...
              style={{
                background: input.trim().length >= 4 ? '#059669' : '#d1d5db',
                color: 'white', border: 'none', borderRadius: 10,
                padding: '12px 28px', fontSize: 15, fontWeight: 700,
                cursor: input.trim().length >= 4 ? 'pointer' : 'not-allowed',
          ...
          <div>
            ...
            <div style={{
              ...
              background: matches.length === 0
                ? '#fef2f2'
                : hasExact ? '#f0fdf4' : '#fffbeb',
              border: `2px solid ${matches.length === 0 ? '#fca5a5' : hasExact ? '#86efac' : '#fde68a'}`,
            }}>
              {matches.length === 0 ? (
                <>
                  <div style={{ fontSize: 28, marginBottom: 6 }}>😔</div>
                  <div style={{ fontSize: 17, fontWeight: 700, color: '#dc2626' }}>
                    No match found for <code style={{ fontFamily: 'monospace' }}>{query}</code>
                  </div>
                  <div style={{ fontSize: 13, color: '#6b7280', marginTop: 6 }}>
                    This ticket number was not found in our current data. Note: we may not have all draws published yet.
                    Always verify at <strong>statelottery.kerala.gov.in</strong>.
                  </div>
                </>
              ) : hasExact ? (
                <>
                  ...
                  <div style={{ fontSize: 17, fontWeight: 700, color: '#059669' }}>
                    Congratulations! Ticket <code style={{ fontFamily: 'monospace' }}>{query}</code> is a winner!
                  </div>
                  <div style={{ fontSize: 13, color: '#065f46', marginTop: 6 }}>
                    Please verify officially at <strong>statelottery.kerala.gov.in</strong> before claiming any prize.
                    Prize must be claimed within 90 days of draw date.
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div style={{ fontSize: 17, fontWeight: 700, color: '#92400e' }}>
                    {matches.length} partial match{matches.length > 1 ? 'es' : ''} found for <code style={{ fontFamily: 'monospace' }}>{query}</code>
                  </div>
                  <div style={{ fontSize: 13, color: '#78350f', marginTop: 6 }}>
                    These tickets end with your number. Enter the full ticket including series letters (e.g. <strong>RR 281074</strong>) to confirm exactly.
                  </div>
                </>
              )}
            </div>
        ...
        <div style={{
          ...
          <strong>⚠️ Important:</strong> This tool searches our published result data only.
          Always verify your winning ticket at the official Kerala State Lotteries website
          (<strong>statelottery.kerala.gov.in</strong>) or in the Kerala Government Gazette before
          claiming any prize. Prizes must be claimed within 90 days of draw date.

...
export function ShareGuessingButton({ day, lotteryName, pageUrl }: Props) {
  const [copied, setCopied] = useState(false);
  const { A, B, C } = day.boards;
  const hotPick = day.numbers.find((n) => n.label === 'Hot Pick')?.value;
  const luckyCombo = day.numbers.find((n) => n.label === 'Lucky Combo')?.value;
  const title = lotteryName ? `${lotteryName} Guessing Numbers` : 'Kerala Lottery Guessing Numbers';

  const message = [
    `🎯 ${title} — ${day.displayLabel}`,
    ``,
    `A: ${A}  B: ${B}  C: ${C}`,
    hotPick ? `🔥 Hot Pick: ${hotPick}` : null,
    luckyCombo ? `🍀 Lucky Combo: ${luckyCombo}` : null,
    ``,
    `Full board & 4-digit picks: ${pageUrl}`,
    ``,
    `⚠️ For entertainment only — not guaranteed winning numbers.`,
  ]
    .filter(Boolean)
    .join('\n');
  ...
  return (
    <div className="share-result-block">
      <div className="share-result-block__label">Share today's guessing numbers</div>

...
export function getResultByDraw(slug: string, drawCode: string) {
  const normalized = drawCode.toLowerCase();
  return results.find((result) => result.lotterySlug === slug && result.drawCode.toLowerCase() === normalized);
}
...
export function drawPath(result: Result) {
  return `/results/${result.lotterySlug}/${result.drawCode.toLowerCase()}`;
}
